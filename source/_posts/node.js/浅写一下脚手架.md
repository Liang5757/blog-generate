...
必须要加第一行 `#! /usr/bin/env node`，将执行环境设置为 node，否则会报错：[相关 StackOverflow 讨论](https://stackoverflow.com/questions/39585342/node-when-i-run-package-json-bin-command-give-me-syntax-error-near-unexp)
...
如果不满足于 commander.js 在一行内拼接大量参数的形式（开发者也难以记住复杂的参数用法），可以用 [Inquirer.js](https://github.com/SBoudrias/Inquirer.js) 来做到交互式的命令行。
...
![交互式命令行效果](浅写一下脚手架/inquirer-demo.png)

## 命令行 loading 效果（ora）

在脚手架执行耗时操作（如下载模板、安装依赖）时，为了提供更好的用户体验，我们需要显示一个 loading 动画，让用户知道程序正在运行中。[ora](https://github.com/sindresorhus/ora) 就是专门用来实现这个功能的库。
...
![loading 效果](浅写一下脚手架/ora-demo.png)

## 执行命令

在脚手架中，我们经常需要执行一些 shell 命令，比如 `git clone` 下载模板、`npm install` 安装依赖等。Node.js 提供了 `child_process` 模块来执行外部命令，但我们更推荐使用 [execa](https://github.com/sindresorhus/execa)，它是 `child_process` 的增强版本，提供了更好的 API 和错误处理。
...
为了让命令行输出更加直观和美观，我们可以使用 [chalk](https://github.com/chalk/chalk) 库为文本添加颜色和样式。不同的颜色可以表示不同的状态：成功用绿色、失败用红色、警告用黄色等。
...
![彩色命令行效果](浅写一下脚手架/chalk-demo.png)

## 下载模板

现在我们已经掌握了命令行的基础能力，接下来实现脚手架的核心功能 —— 从 Git 仓库下载项目模板。[download-git-repo](https://github.com/flipxfx/download-git-repo) 是一个专门用于下载 Git 仓库的库。
...
下载完模板后，我们通常需要对文件进行一些操作：复制文件、删除文件、创建目录等。Node.js 原生的 `fs` 模块虽然功能强大，但 API 比较原始。[fs-extra](https://github.com/jprichardson/node-fs-extra) 是 `fs` 的增强版本，提供了更多实用的方法。
...
在实际项目中，我们通常需要根据用户的选择动态生成配置文件，比如 `package.json`、`.eslintrc` 等。这时可以使用模板引擎来实现。[ejs](https://github.com/mde/ejs) 是一个简单易用的模板引擎。
...
![脚手架完整创建流程](浅写一下脚手架/complete-flow-demo.png)
...
### 1. 为什么运行 `npm link` 后命令找不到？

**问题**：执行 `npm link` 后，在终端输入命令提示 "command not found"。

**解决方案**：
- 检查 `package.json` 中 `bin` 字段配置是否正确
